/* =====================================================================
   VW_MKT_OPORTUNIDADES - MAESTRO v3 (11 esquemas)
   Novedad v3: se agrega la INDUSTRIA DE LA OPORTUNIDAD (OOPR.Industry),
   ademas de la del cliente (OCRD.IndustryC). Son columnas separadas:
     CodIndustria / NomIndustria           -> del cliente (OCRD)
     CodIndustriaOport / NomIndustriaOport -> de la oportunidad (OOPR)
   Mantiene todo lo de v2 (NroAsunto, Sucursal OUBR/fallback, sin razon).

   Grupos por U_:
     A) COMPLETO: Rental, Argentina, Peru, Colombia, Paraguay, Uruguay, Ecuador
     B) SIN U_TipoNegocio: Training, Panama, Ventas
     C) SIN NINGUN U_: Logistic

   OJO: agrega 2 columnas mas vs v2 -> re-ejecutar el PASO 1 del script
   de SQL Server (la tabla se auto-infiere y el SP no cambia).
   ===================================================================== */
